  ...
  return (
    <div className="pdf-report" dir="rtl">
            ...
            {report.mainPhotos.map((photo, index) => (
              <div key={photo.localId} className="pdf-photo-item">
                {photo.uploadedUrl ? (
                  ...
                  <div className="pdf-photo-link">
                    <img
                      src={photo.previewUrl || URL.createObjectURL(photo.file)}
                      alt={`Photo ${index + 1}`}
                      className="pdf-photo-thumbnail"
                    />
                    <div className="pdf-photo-label">صورة {index + 1} (غير مرفوعة)</div>
                  </div>
                )}
              </div>
            ))}
            ...
            {report.complaintFiles.map((file, index) => (
              <div key={file.localId} className="pdf-file-item">
                ...
                {file.uploadedUrl ? (
                  ...
                  <span className="pdf-file-name">{file.name} (غير مرفوع)</span>
                )}
                      ...
                      {photo.uploadedUrl ? (
                        ...
                        <div className="pdf-photo-link">
                          <img
                            ...
                            className="pdf-photo-thumbnail"
                          />
                          <div className="pdf-photo-label">صورة {photoIndex + 1} (غير مرفوعة)</div>
                        </div>
                      )}
                    </div>
                  ))}
      ...
      <div className="pdf-footer">
        <p>تم إنشاء هذا التقرير بواسطة نظام VisitProp</p>
        <p>Generated by VisitProp System</p>
      </div>
    </div>
  );
}
